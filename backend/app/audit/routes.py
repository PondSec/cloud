...
def _filtered_event_query():
    ...
    if from_dt is not None:
        query = query.filter(AuditEvent.ts >= from_dt)
    if to_dt is not None:
        query = query.filter(AuditEvent.ts <= to_dt)
    if action:
        query = query.filter(AuditEvent.action == action)
    if user_id not in (None, ""):
        try:
            query = query.filter(AuditEvent.actor_user_id == int(user_id))
        except ValueError as error:
            raise APIError(400, "INVALID_PARAMETER", "user_id must be an integer.") from error
    if success is not None:
    ...
    if q:
        ...
        query = query.filter(
            or_(
                AuditEvent.action.ilike(wildcard),
                AuditEvent.entity_type.ilike(wildcard),
                AuditEvent.entity_id.ilike(wildcard),
                cast(AuditEvent.metadata_json, db.String).ilike(wildcard),
            )
        )
...
@audit_bp.get("/events")
def list_events():
    query = _filtered_event_query()

    export = (request.args.get("export") or "").strip().lower()
    if export == "csv":
        rows = query.order_by(AuditEvent.ts.desc(), AuditEvent.id.desc()).limit(10000).all()
        buffer = io.StringIO()
        writer = csv.writer(buffer)
        writer.writerow(
            [
        ...
        return (
            ...
            {
                "Content-Type": "text/csv; charset=utf-8",
                "Content-Disposition": f'attachment; filename="audit-events-{datetime.now(timezone.utc).strftime("%Y%m%d%H%M%S")}.csv"',
            },
        )
    ...
    total = query.count()
    items = (
        query.order_by(AuditEvent.ts.desc(), AuditEvent.id.desc())
        .offset((page - 1) * page_size)
        .limit(page_size)
        .all()
    )
...
@audit_bp.get("/events/actions")
def list_event_actions():
    actions = [
        row[0]
        for row in db.session.query(distinct(AuditEvent.action))
        .filter(AuditEvent.action.isnot(None))
