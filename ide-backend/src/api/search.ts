...
type FileSearchResponse = { items: string[] };
type TextSearchMatch = { path: string; line: number; column: number; preview: string; match: string };
type TextSearchResponse = { items: TextSearchMatch[]; truncated: boolean };
...
type CacheEntry = { expiresAt: number; files: string[] };
...
searchRouter.get('/:workspaceId/files', async (req, res, next) => {
  try {
    ...
    if (!q) {
      const emptyResponse: FileSearchResponse = { items: [] };
    ...
    const response: FileSearchResponse = { items: results };
    res.json(response);
  } catch (error) {
    next(error);
  }
...
searchRouter.post('/:workspaceId/text', async (req, res, next) => {
  try {
    ...
    const response: TextSearchResponse = { items, truncated };
    res.json(response);
  } catch (error) {
    next(error);
  }
...
async function getWorkspaceFileIndex(workspaceId: string, root: string): Promise<string[]> {
  const now = Date.now();
...
async function listFilesPreferRipgrep(root: string): Promise<string[]> {
  try {
...
async function walkFiles(root: string, relDir: string, out: string[]): Promise<void> {
...
function shouldExcludePath(relPosix: string): boolean {
...
async function runRipgrepFiles(root: string): Promise<string[]> {
...
function rankFiles(files: string[], query: string, limit: number): string[] {
...
function fuzzyScore(candidate: string, q: string): number | null {
...
async function searchText(args: {
  ...
  exclude: string[];
  maxResults: number;
}): Promise<{ items: TextSearchMatch[]; truncated: boolean }> {
  try {
...
async function searchTextWithRipgrep(args: {
  ...
  const rgArgs: string[] = ['--json', '--color', 'never', '--max-filesize', '2M'];
  ...
  const processLine = (line: string) => {
    ...
    const data = msg.data;
    if (!data?.path?.text || typeof data?.line_number !== 'number' || !data?.lines?.text) return;
    ...
    const lineNo = data.line_number as number;

    const submatches = Array.isArray(data.submatches) ? data.submatches : [];
    for (const sm of submatches) {
      if (!sm || typeof sm.start !== 'number' || !sm.match?.text) continue;
  ...
  const done = new Promise<{ items: TextSearchMatch[]; truncated: boolean }>((resolve, reject) => {
...
async function searchTextFallback(args: {
  ...
  const filtered = files.filter((p) => {
    if (shouldExcludePath(p)) return false;
    for (const ex of args.exclude) {
      if (p.includes(ex.replace('/**', ''))) return false;
    }
  ...
  for (const rel of filtered) {
    ...
    if (stat.size > MAX_FILE_BYTES_FALLBACK) continue;
...
async function spawnCapture(
  bin: string,
  args: string[],
  options: { cwd: string; maxStdoutBytes: number },
  ...
  const done = new Promise<{ stdout: string; stderr: string; exitCode: number }>((resolve, reject) => {
    child.stdout.on('data', (chunk) => {
...
function isSpawnMissingBinary(error: unknown): boolean {
  return (
    typeof error === 'object' &&
    error !== null &&
    'code' in error &&
