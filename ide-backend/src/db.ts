...
import type { UserRecord, WorkspaceRecord, WorkspaceSettings } from './types.js';
...
export function createWorkspace(record: WorkspaceRecord, settings: WorkspaceSettings): void {
  const insertWorkspace = db.prepare(
    `INSERT INTO workspaces (id, user_id, name, template, created_at, updated_at) VALUES (@id, @userId, @name, @template, @createdAt, @updatedAt)`,
  );
...
export function listWorkspaces(userId: string): WorkspaceRecord[] {
  const rows = db
    .prepare(
      `SELECT id, user_id AS userId, name, template, created_at AS createdAt, updated_at AS updatedAt
...
export function findWorkspace(workspaceId: string, userId: string): WorkspaceRecord | undefined {
  const row = db
    .prepare(
      `SELECT id, user_id AS userId, name, template, created_at AS createdAt, updated_at AS updatedAt
  ...
  return row as WorkspaceRecord | undefined;
}
...
export function getWorkspaceSettings(workspaceId: string): WorkspaceSettings {
  const row = db.prepare(`SELECT settings_json FROM workspace_settings WHERE workspace_id = ?`).get(workspaceId) as
...
export function setWorkspaceSettings(workspaceId: string, settings: WorkspaceSettings): void {
  db.prepare(`UPDATE workspace_settings SET settings_json = ? WHERE workspace_id = ?`).run(
...
export function upsertGitCredential(record: GitCredentialRecord): void {
  db.prepare(
    `INSERT INTO git_credentials (id, workspace_id, host, username, ciphertext, iv, auth_tag, created_at, updated_at)
...
export function findGitCredential(workspaceId: string, host: string): GitCredentialRecord | undefined {
  const row = db
    .prepare(
      `SELECT id, workspace_id AS workspaceId, host, username, ciphertext, iv, auth_tag AS authTag,
              created_at AS createdAt, updated_at AS updatedAt
       FROM git_credentials WHERE workspace_id = ? AND host = ?`,
    )
    .get(workspaceId, host);
  return row as GitCredentialRecord | undefined;
}

export function deleteGitCredential(workspaceId: string, host: string): void {
